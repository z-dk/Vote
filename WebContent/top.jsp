...
<body>
					...
					<form class="form-horizontal">
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input type="text" name="userName" class="form-control"
									id="userName3" placeholder="UserName"> <span
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input type="text" name="phoneNum" class="form-control"
									id="phoneNum3" placeholder="phoneNum">
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input type="password" class="form-control" id="passwordInput3"
									placeholder="Password" name="userPassword"> <span
									class="help-block"></span>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input type="password" class="form-control"
									id="passwordconfirm3" placeholder="Password"> <span
									class="help-block"></span>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<div class="col-sm-offset-9 col-sm-3">
								<button type="button" class="btn btn-primary" id="commit3"
									onclick="">提交</button>
	...
	<!--登陆时所弹出的模态框 -->
	<div class="modal fade" id="userLoginInModal" tabindex="-1"
				...
				<div class="modal-body">
					<form class="form-horizontal">
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input type="text" name="userName" class="form-control"
									id="inputUsername" placeholder="UserName"> <span
									class="help-block"></span>
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<div class="col-sm-offset-9 col-sm-3">
								<button type="button" class="btn btn-primary" id="log_in"
	...
	<!-- 注册时弹出的模态框 -->
	<div class="modal fade" id="RegisterInModal" tabindex="-1"
		role="dialog" aria-labelledby="myModalLabel" data-backdrop="static">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div class="modal-body">
					<form>
						<div class="form-group">
							<label for="col-sm-2 control-label">用户名</label> <input
						...
						<div class="form-group">
							<label for="col-sm-2 control-label">电话号码</label> <input
								type="text" class="form-control" id="teleNumberInput"
								placeholder="Telephonenumber" name="phoneNum"> <span
								class="help-block"></span>
						</div>
	...
	<%-- --%>
	<div
		style="height:700px;background-image:url('${APP_PATH }/static/images/top.jpg');  background-size:100%100%">
		<div class="header">
			<div style="height: 20px"></div>
			<div class="row">
				<div class="col-sm-2"></div>
				<div class="col-sm-1">
					<button type="button" class="btn btn-index" id="to_Characteristic">特色介绍</button>
				</div>
				<div class="col-sm-1">
					<button type="button" class="btn btn-index" id="to_case">经典案例</button>
				</div>
				<div class="col-sm-1">
					<button type="button" class="btn btn-index" id="helpCenter">帮助中心</button>
				</div>
				<div class="col-sm-1">
					<button type="button" class="btn btn-index" id="to_bottom">合作品牌</button>
				</div>
				<div class="col-sm-2"></div>
				<div class="col-sm-1" id="registerdiv">
					<button type="button" class="btn btn-block btn-index"
						data-toggle="modal" data-target="#RegisterInModal"
		...
		<div class="row">
			...
			<h1 align="center">
				<font color="white">更简单的操作、更多用户的选择、更强大的功能</font>
			</h1>
	...
	<script type="text/javascript">
		...
		$("#Createbtn").click(function() {
			if ($(this).attr("code") == "error")
				...
				window.location.href = "${APP_PATH}/user/createvote?userId="
		...
		function build_login() {
			$("#LoginChange").empty();
			$("<button></button>")
					.addClass("btn btn-block btn-index login_btn").attr(
							"data-toggle", "modal").attr("data-target",
							"#userLoginInModal").append("登录").appendTo(
							"#LoginChange");
		...
		function build_successLogin() {
			$("#LoginChange").empty();
			var a = $("<li></li>").append(
					$("<a></a>").append("个人中心").attr(
							"href",
							"${APP_PATH}/user/success?userName=" + userName
									+ "&userId=" + userId));
			var a1 = $("<a></a>").append("退出登录").attr("href",
					"${APP_PATH }/user/exit");
			var b = $("<li></li>").append(a1);
			var ul = $("<ul></ul>").addClass("dropdown-menu").append(a).append(
					b);
			var button = $("<button onclick='dropdown()'></button>").addClass(
					"btn btn-index dropdown-toggle").append(userName).attr(
					"data-toggle", "dropdown").attr("aria-haspopup", "true")
					.attr("aria-expanded", "false").append(
		...
		$("#userInput").change(
				function() {
					...
					$.ajax({
						...
						success : function(result) {
							if (result.code == 100) {
								show_validate_msg("#userInput", "success",
										"用户名可用");
								$("#Register").attr("ajax-va", "success");
							} else {
								show_validate_msg("#userInput", "error",
		...
		$("#passwordInput").change(function() {
			...
			if (!regpassword.test(password)) {
				show_validate_msg("#passwordInput", "error",
						"密码为6-16位，且不能有非法字符");
				$("#Register").attr("ajax-va", "error");
			} else {
					...
					if (result.code == 100) {
						...
						if (undefined != result.extend.errorFields.userPassword) {
							show_validate_msg(
									"#passwordInput",
									"error",
									result.extend.errorFields.userPassword);
						}
						if (undefined != result.extend.errorFields.phoneNum) {
							show_validate_msg(
									"#teleNumberInput",
									"error",
									result.extend.errorFields.phoneNum);
						}
			...
			$.ajax({
				...
				success : function(result) {
					...
					if (result.code == 100) {
						...
						alert(result.extend.errorFields.userPassword);
						reset_form("#findpwdModal form");
					} else {
						alert("用户名或手机号码验证失败！");
						reset_form("#findpwdModal form");
		...
		$("#userLoginInModal").bind('hide.bs.modal', function() {
			$(".modal-backdrop").remove();
		})
		$("#RegisterInModal").bind('hide.bs.modal', function() {
			$(".modal-backdrop").remove();
		})
		$("#findpwdModal").bind('hide.bs.modal', function() {
			$(".modal-backdrop").remove();
		})
		function dropdown(){
